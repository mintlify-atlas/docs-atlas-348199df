...
sidebarTitle: "Topics"
description: "Reference guide for all WebSocket channels available across Coinbase Advanced Trade, Exchange, International, and Prime — with subscription examples and payload shapes."
---
...
<Note>
  The International Exchange WebSocket feed always requires authentication — even for market data channels. Subscribe using `SUBSCRIBE` (uppercase) type which is handled automatically by the SDK.
</Note>

### `INSTRUMENTS`

Streams real-time instrument (perpetual futures / spot) market data including quotes, best bid/ask, and funding rates.
...
Streams updates to your open orders on the International Exchange.
...
Streams real-time position updates for your portfolio.

```typescript
wsClient.subscribe(
  {
    topic: 'POSITIONS',
    payload: { portfolio: 'your-portfolio-uuid' },
  },
  WS_KEY_MAP.internationalMarketData
);
```
...
Streams funding rate updates for perpetual futures instruments.
...
Streams portfolio-level risk metrics (margin, collateral, etc.).
...
| `internationalMarketData` | `INSTRUMENTS` | 🔒 | Quote + funding data |
| `internationalMarketData` | `ORDERS` | 🔒 | My open orders |
| `internationalMarketData` | `POSITIONS` | 🔒 | Portfolio positions |
| `internationalMarketData` | `FUNDING` | 🔒 | Funding rate updates |
| `internationalMarketData` | `RISK` | 🔒 | Portfolio risk metrics |
